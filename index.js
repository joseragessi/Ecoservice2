...
const { procesarMensaje, mensajeApp } = require('./conversacion');
...
const { iniciarStock, tieneSesionActiva: tieneSesionStock,
        continuarStock, tienePedidoPendiente,
        _esListadoCompleto: esListadoCompletoStock } = require('./stock');
  ...
  try {
    ...
    if (esProveedores) {
      ...
      if (numMedia > 0) {
        ...
        // Una foto de remito de combustible ya no se procesa por el bot: el
        // capataz tiene que cargarla en la app, donde elige la MÁQUINA real y
        // no puede cargar sin tener el censo del mes (decisión 10-sep). Se lo
        // manda a la app en vez de procesar una carga que va a quedar sin
        // imputar a ninguna máquina.
        if (!respuesta) respuesta = await avisarComboEnApp(telefono);
      ...
      } else if (await tienePedidoPendiente(telefono)) {
        // Se le pidió el stock y todavía no contestó. La plantilla de WhatsApp
        // (la única que se puede mandar fuera de las 24 hs) no puede llevar el
        // listado adentro, así que la respuesta —sea la que sea— es lo que
        // abre la ventana: acá se le manda el listado para confirmar.
        // Solo se procesa como listado si es un listado COMPLETO (5+ renglones
        // o un texto largo). Con un criterio más flojo, un reporte como
        // "se rompió la motoguadaña 12" —que tiene un equipo y un número—
        // pisaba el stock del objetivo. Todo lo demás muestra el listado para
        // confirmar, y desde ahí el capataz corrige o escribe *menu* para salir.
        const t = mensaje.trim();
        respuesta = await iniciarStock(telefono, esListadoCompletoStock(t) ? t : '');
      } else {
...
// El capataz mandó una foto de un remito: se le contesta con el link de la
// app y su usuario, en vez de procesarla acá.
async function avisarComboEnApp(telefono) {
  try {
    const supabase = require('./supabase');
    const tel = String(telefono || '').replace('whatsapp:', '').replace('+', '');
    const { data: cap } = await supabase.from('capataces')
      .select('id, nombre, usuario').eq('telefono', tel).eq('activo', true).maybeSingle();
    return `📷 Recibí la foto, pero el combustible ahora se carga desde la app.\n\n` + mensajeApp(cap || {});
  } catch (e) {
    console.error('[combustible] aviso app:', e.message);
    return '📷 Recibí la foto, pero el combustible ahora se carga desde la app. Escribile a Logística si no tenés el acceso.';
  }
}

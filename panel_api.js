...
.accion-card{display:flex;align-items:center;gap:12px;background:var(--papel);border:1px solid var(--linea);border-radius:10px;padding:12px 14px;transition:border-color .15s}
.accion-card:hover{border-color:var(--brote)}
.accion-n{font-size:22px;font-weight:700;font-family:var(--mono,monospace)}
.accion-t{font-weight:600;font-size:13px}
...
<div id="app">
  <aside class="rail">
    ...
    <nav class="nav" id="nav">
      ...
      <div class="nav-item" data-v="stock" onclick="go('stock')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 7h16M4 7l2-3h12l2 3M4 7v13h16V7"/><path d="M9 11h6M9 15h6"/></svg>Stock</div>
...
<div class="modal-bg" id="mm-bg">
  <div class="modal">
    ...
    <div class="modal-acciones" id="mm-acciones">
...
async function iniciar(){
  ...
  const orden=['dashboard','facturas','insumos','combustible','compras','reparaciones','stock','maestros'];
...
/* ===== Navegación ===== */
const CRUMB={dashboard:'Dashboard',facturas:'Facturas',insumos:'Insumos',combustible:'Combustible',reparaciones:'Reparaciones',maestros:'Maestros',compras:'Compras',stock:'Stock'};
...
async function vCombustible(view){
  const tabs=`<div class="toggle-imp" style="margin-bottom:16px">
    ...
    <button class="${combTab==='viajes'?'on':''}" onclick="combTab='viajes';go('combustible')">Viajes · bateas</button>
  </div>`;
  if(combTab==='analisis'){return vCombAnalisis(view,tabs);}
  if(combTab==='viajes'){return vViajes(view,tabs);}
  try{
...
/* ── Combustible · Viajes / bateas (roll off) ── */
let viajesDesde='', viajesHasta='';
async function vViajes(view,tabs){
  view.innerHTML=tabs+'<div class="cargando-v">Cargando…</div>';
  ...
  try{
    ...
    view.innerHTML=`
    <div class="view-head"><div><div class="view-title">Combustible</div>
      <div class="view-desc">Viajes de roll off · km, bateas y puntos de bajada por chofer</div></div></div>
    ${tabs}
    <div style="display:flex;gap:8px;align-items:center;margin-bottom:14px">
      <span class="sub">Desde</span><input type="date" value="${desde}" onchange="viajesDesde=this.value;go('combustible')" style="padding:6px;border:1px solid var(--linea);border-radius:8px">
      <span class="sub">hasta</span><input type="date" value="${hasta}" onchange="viajesHasta=this.value;go('combustible')" style="padding:6px;border:1px solid var(--linea);border-radius:8px">
    </div>
    <div class="kpis" style="grid-template-columns:repeat(4,1fr);margin-bottom:12px">
      ${kpi('KM recorridos',(k.km_total||0).toLocaleString('es-AR')+' km',k.jornadas+' jornada(s)')}
      ${kpi('Bateas trasladadas',k.bateas_total||0,(k.bateas_por_jornada||0)+' por jornada')}
      ${kpi('Puntos de bajada',k.puntos_total||0,'objetivos atendidos')}
      ${kpi('KM por batea',(k.km_por_batea!=null?k.km_por_batea:'—'),'eficiencia de traslado')}
    </div>
    <div class="grid" style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:16px">
      <div class="panel"><div class="panel-title" style="margin-bottom:10px">Por chofer</div>
        ${(ind.por_chofer||[]).length?`<div class="tablewrap"><table><thead><tr><th>Chofer</th><th class="num">KM</th><th class="num">Bateas</th><th class="num">Puntos</th><th class="num">Jorn.</th><th class="num">km/batea</th></tr></thead><tbody>
          ${ind.por_chofer.map(c=>`<tr><td>${c.chofer}</td><td class="num mono">${c.km.toLocaleString('es-AR')}</td><td class="num mono">${c.bateas}</td><td class="num mono">${c.puntos}</td><td class="num mono">${c.jornadas}</td><td class="num mono">${c.km_por_batea??'—'}</td></tr>`).join('')}
    ...
    <div class="tablewrap"><table><thead><tr><th>Fecha</th><th>Chofer</th><th>Unidad</th><th class="num">Odóm. ini</th><th class="num">Odóm. fin</th><th class="num">KM</th><th class="num">Bateas</th><th>Paradas</th><th></th></tr></thead>
      <tbody>${lista.length?lista.map(v=>`<tr>
        ...
        :'<tr><td colspan="9"><div class="empty" style="height:120px"><div>No hay viajes cargados en el período.<br><span class="sub">Los choferes los cargan por WhatsApp al bot escribiendo "viajes".</span></div></div></td></tr>'}</tbody></table></div>`;
  }catch(e){view.innerHTML=tabs+`<div class="cargando-v">${e.message||'No pude cargar los viajes.'}<br><span class="sub">¿Creaste la tabla viajes_bateas en Supabase?</span></div>`;}
}
async function anularViaje(id){
  ...
  try{await api('/api/viajes/'+id+'/anular',{method:'POST',body:'{}'});go('combustible');}
...
function vRepInd(view){
  ...
  // Tiempo de resolución: creada → finalizada
  ...
  const agr=(lista,clave)=>{
    const m={};lista.forEach(r=>{const k=clave(r)||'Sin asignar';m[k]=(m[k]||0)+1;});
    return Object.entries(m).map(([nombre,valor])=>({nombre,valor})).sort((a,b)=>b.valor-a.valor);
  };
  // Por mecánico: activas, finalizadas y tiempo promedio propio
  const mecs={};
  fs.forEach(r=>{
    const k=r.mecanicos?r.mecanicos.nombre:'Sin asignar';
    mecs[k]=mecs[k]||{activas:0,finalizadas:0,tiempos:[]};
    if(r.estado==='finalizado'){mecs[k].finalizadas++;
  ...
  // Evolución: nuevas vs finalizadas por mes (sobre todas)
  const porMes={};
  todas.forEach(r=>{
    const mN=mesDe(r.created_at);porMes[mN]=porMes[mN]||{nuevas:0,fin:0};porMes[mN].nuevas++;
    if(r.estado==='finalizado'&&r.fecha_finalizado){const mF=mesDe(r.fecha_finalizado);
      porMes[mF]=porMes[mF]||{nuevas:0,fin:0};porMes[mF].fin++;}
  });
  const evol=Object.entries(porMes).filter(([m])=>m!=='sin fecha').sort((a,b)=>a[0].localeCompare(b[0]));
  view.innerHTML=`
  <div class="view-head"><div><div class="view-title">Reparaciones · Indicadores</div>
    <div class="view-desc">Carga del taller, tiempos de resolución y distribución</div></div>
  ...
  ${tabsRep()}
  <div class="kpis" style="grid-template-columns:repeat(4,1fr)">
    <div class="kpi ${activas.length?'amber':''}"><div class="kpi-label">Activas</div><div class="kpi-val ${activas.length?'amber':''}">${activas.length}</div><div class="kpi-sub">en el taller ahora</div></div>
    <div class="kpi"><div class="kpi-label">Finalizadas</div><div class="kpi-val green">${finalizadas.length}</div><div class="kpi-sub">${repIndPer?'creadas en '+mesStk(repIndPer):'histórico'}</div></div>
    <div class="kpi plain"><div class="kpi-label">Resolución promedio</div><div class="kpi-val">${tProm!=null?Math.round(tProm*10)/10:'—'}</div><div class="kpi-sub">días de creada a finalizada</div></div>
    <div class="kpi ${criticasAltas?'':'plain'}"><div class="kpi-label">Crítico + alta</div><div class="kpi-val ${criticasAltas?'amber':''}">${criticasAltas}</div><div class="kpi-sub">activas con prioridad</div></div>
  </div>
  <div class="grid g-2" style="margin-bottom:18px">
    <div class="panel"><div class="panel-title">Por mecánico</div>
      <table style="font-size:12px"><thead><tr><th>Mecánico</th><th class="num">Activas</th><th class="num">Finalizadas</th><th class="num">Resol. prom.</th></tr></thead>
      <tbody>${tablaMec||'<tr><td colspan="4" class="sub" style="padding:10px">Sin datos</td></tr>'}</tbody></table>
    </div>
    <div class="panel"><div class="panel-title">Evolución mensual</div>
      <table style="font-size:12px"><thead><tr><th>Período</th><th class="num">Nuevas</th><th class="num">Finalizadas</th><th class="num">Balance</th></tr></thead>
      <tbody>${evol.map(([m,v])=>{const bal=v.fin-v.nuevas;
        return `<tr><td class="mono">${m}</td><td class="num">${v.nuevas}</td><td class="num">${v.fin}</td>
        <td class="num" style="${bal<0?'color:var(--rojo)':'color:var(--brote-2)'}">${bal>0?'+':''}${bal}</td></tr>`;}).join('')}</tbody></table>
    </div>
  </div>
  <div class="grid g-2">
    <div class="panel"><div class="panel-title">Por tipo de equipo</div>${barsGen(agr(fs,r=>r.equipos?(r.equipos.tipo||r.equipos.nombre):null),'var(--brote)')}</div>
    <div class="panel"><div class="panel-title">Por objetivo</div>${barsGen(agr(fs,r=>r.objetivos?r.objetivos.nombre:null),'var(--diesel)')}</div>
  </div>`;
}
...
const MODS_PANEL=[['dashboard','Dashboard'],['facturas','Facturas'],['insumos','Insumos'],['combustible','Combustible'],['compras','Compras'],['reparaciones','Reparaciones'],['stock','Stock'],['maestros','Maestros']];
...
function cardMaestro(m,ix){
  ...
  if(maestroTab==='unidades'){
    ...
    extra=`<div class="mcard-row"><span>Patente</span><b class="mono">${m.patente||'—'}</b></div>
...
function abrirModalMaestro(m){
  ...
  if(maestroTab==='mecanicos'){
    ...
    campos+=`<div class="divider"></div>
      ...
      <div class="mm-field"><label>Rol en la app</label><select id="mm-rolapp">
        ...
        <option value="supervisor" ${m.rol_app==='supervisor'?'selected':''}>Supervisor — ve todos los pedidos de insumos (solo lectura)</option>
      </select></div>
...
async function guardarMaestro(){
  ...
  if(maestroTab==='mecanicos'){
    body.habilidades=[...document.querySelectorAll('#mm-campos .mm-habs input[type=checkbox]:checked')].map(c=>c.value);
